...
import { X, Download, FileText, Image as ImageIcon, FileCode, File, Loader2 } from 'lucide-react';
...
export function FileViewerModal({ mode, projectId, sessionId, filePath, onClose }: FileViewerModalProps) {
  ...
  const getFileUrl = () => {
    ...
    return mode === 'project'
      ? `${config.apiUrl}/projects/${projectId}/files/download?file_path=${encodedPath}`
      : `${config.apiUrl}/sessions/${sessionId}/files/download?file_path=${encodedPath}`;
  };
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-none lg:rounded-lg shadow-2xl w-full h-full lg:max-w-5xl lg:h-[85vh] flex flex-col"
        ...
        <div className="flex-1 overflow-auto p-6 bg-white">
          ...
          {!loading && !error && (
            <>
              ...
              {/* Code Viewer */}
              {fileInfo?.type === 'code' && (
                <div className="bg-primary-100 rounded-lg border border-primary-200 overflow-hidden">
                  <pre className="font-mono text-sm text-gray-900 overflow-x-auto">
                    {content.split('\n').map((line, i) => (
                      <div key={i} className="flex hover:bg-gray-100">
                        <span className="inline-block w-12 text-right pr-3 text-gray-400 select-none flex-shrink-0 border-r border-primary-200 bg-gray-50">
                          {i + 1}
                        </span>
              ...
              {/* Text Viewer */}
              {fileInfo?.type === 'text' && (
                <pre className="font-mono text-sm text-gray-900 whitespace-pre-wrap bg-primary-100 p-4 rounded-lg border border-primary-200">
                  {content}
                </pre>
              )}
              ...
              {fileInfo?.type === 'binary' && (
                <div className="text-center text-gray-500 py-12">
                  <File className="w-8 h-8 mx-auto mb-4 text-gray-400" />
                  <p className="text-lg font-medium">Cannot preview this file type</p>
                  <p className="text-sm mt-2">Click the download button to view it locally</p>
                </div>
              )}
            </>
          )}
